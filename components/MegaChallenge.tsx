            ...
            <div className="space-y-6 animate-fade-in max-w-4xl mx-auto">
               {(megaData.multipleChoice || []).map((q, idx) => (
                  <div key={q.id} className="bg-white p-6 rounded-[2rem] shadow-lg border-2 border-slate-50 transition-all hover:border-brand-200">
                    ...
                    {submitted[q.id] && (
                        <p className="mt-3 p-3 rounded-xl bg-slate-50 text-slate-600 italic font-bold text-xs">"{q.explanation}"</p>
                    )}
                  </div>
               ))}
            </div>
                    ...
                    return (
                        <div key={q.id} className="bg-white p-8 rounded-[2rem] shadow-lg border-b-4 border-slate-100 flex flex-col gap-4">
                            ...
                            {!submitted[q.id] ? (
                                ...
                                <div className={`p-4 rounded-xl flex items-center gap-3 ${isAllCorrect ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
                                    <span className="text-3xl">{isAllCorrect ? '🌟' : '💡'}</span>
                                    <div>
                                        <p className="font-black text-sm">{isAllCorrect ? 'Tuyệt vời!' : `Đáp án đúng: ${q.correctAnswer}`}</p>
              ...
              {(megaData.errorId || []).map((q, idx) => {
                  ...
                  return (
                    <div key={q.id} className="bg-white p-8 rounded-[2rem] shadow-lg border-b-4 border-slate-100 flex flex-col gap-6">
                        ...
                        {submitted[q.id] && (
                            <div className={`p-4 rounded-xl text-center border-t animate-fade-in ${isCorrect ? 'text-green-700 bg-green-50/50' : 'text-rose-700 bg-rose-50/50'}`}>
                                <p className="font-black text-lg mb-1">{isCorrect ? 'Chính xác! 🏆' : 'Chưa đúng rồi!'}</p>
                                <p className="font-bold italic text-slate-500 text-xs">"{q.explanation}"</p>
                            </div>
                        )}
                    </div>
                  );
                    ...
                    return (
                        <div key={q.id} className="bg-white p-8 rounded-[2rem] shadow-lg border-b-4 border-slate-100 flex flex-col gap-4">
                            ...
                            {!submitted[q.id] ? (
                                ...
                                <div className={`p-4 rounded-xl flex items-center gap-3 ${isCorrect ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
                                    <div>
                                        <p className="font-black text-sm">{isCorrect ? 'Tuyệt vời!' : `Đáp án đúng: ${q.correctSentence}`}</p>
                                        {q.translation && <p className="text-[10px] italic opacity-80 mt-1">({q.translation})</p>}
                                    </div>
                                </div>
                            )}
                        </div>
